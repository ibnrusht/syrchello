...
def evaluate(model, char_to_idx, idx_to_char, start_text='. ', prediction_len=100, temp=0.3):
    ...
    while (predicted_char not in punctuation):
...
conf_path = Path('/srv/syr-conf')

with (conf_path / 'char_to_idx.pickle').open("rb") as f:
    char_to_idx = pickle.load(f)
    
with (conf_path / 'idx_to_char.pickle').open("rb") as f:
    idx_to_char = pickle.load(f)

model = RNN(input_size=len(idx_to_char), hidden_size=512, embedding_size=64, n_layers=3)
model.load_state_dict(torch.load(conf_path / 'syrchello.pt'))
